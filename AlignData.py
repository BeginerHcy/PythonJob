...
from AlignFile import Ui_MainWindow
import sys
import os
import csv
import numpy as np
...
class MainWindow(QtWidgets.QMainWindow, Ui_MainWindow):
    ...
    def maincyctask(self):
        ...
        if self.decode == 1:
            if self.initplot == 1:
                fig, ax = plt.subplots()
                ax.set_aspect('equal')  # 确保横纵比例相同
                agx = QDesktopWidget().availableGeometry()
                xx = int(agx.width() / 2 - (self.width() + 800) / 2)
                yy = int(agx.height() / 2 - self.height() / 2 - 31)
                plt.get_current_fig_manager().window.setGeometry(xx + self.width() + 5, yy + 31 + int(0 * 300), 800, 800)
                self.initplot = 0
            ...
            for i in range(self.startfle, self.endfle):
                ...
                plt.scatter(xx, yy, alpha=0.5, color='black', s=0.6, zorder=1)
                # 取数据
            ...
            if self.cyccnt >= self.cyctime:
                ...
                numSlot,datsita1,datr_dat1,ret1,strtime = self.readStdOfOneFile(strpathfileMax)
                numSlot,datsita2,datr_dat2,ret2,strtime = self.readStdOfOneFile(strpathfileMin)

                print(strpathfileMax)
                print(strpathfileMin)

                self.show2Circle2(datsita1,datr_dat1,datsita2,datr_dat2 , ret1, ret2)
    ...
    def showDat(self,index):
        if self.decodefinish == 1 :
            self.plotxy(self.hdat1, self.hdat2, 0,1)
            if index.column() == 0:
    ...
    def plotxy(self,xval,val,pos,equal):
        # plot method
        # ---< ,set plot space
        fig, ax = plt.subplots()
        if equal==1:
        ...
        # ---< ,plot data
        plt.scatter(xval, val, alpha=0.5, color='black', s=6.8, zorder=1)
        plt.grid()
        plt.show(block=False)
    ...
    def plot2xy(self,xval1,val1,xval2,val2,pos):
        ...
        plt.get_current_fig_manager().window.setGeometry(xx + self.width() + 5, yy + 31 + int(pos * 300), 800, 800)
        # ---< ,plot data
        plt.scatter(xval1, val1, alpha=0.5, color='black', s=6.8, zorder=1)
        plt.scatter(xval2, val2, alpha=0.5, color='blue', s=6.8, zorder=1)
        plt.grid()
        plt.show(block=False)
    ...
    def showCircle(self,datsita,datr_dat):
        ...
        self.plotxy(datsita,datr_dat,0,0)
        self.plotxy(xx,yy,1,1)
    ...
    def show2Circle2(self,datsita1,datr_dat1,datsita2,datr_dat2,ret1,ret2):
        xx=[]
        yy=[]
        xx1=[]
        yy1=[]
        ...
        for ii in range(0,len(datsita1)):
            ...
            x = np.cos(datsita1[ii] / 100.0 / 180.0 * 3.141592653) * datr_dat1[ii] / 1000.0
            y = np.sin(datsita1[ii] / 100.0 / 180.0 * 3.141592653) * datr_dat1[ii] / 1000.0
            ...
            x = x - ret1[0]
            y = y - ret1[1]
            c = ret1[2]  / 180.0 * 3.141592653
            _x = x * np.cos(c) + y * np.sin(c)
            _y = y * np.cos(c) - x * np.sin(c)
        ...
        for ii in range(0,len(datsita2)):
            ...
            x = np.cos(datsita2[ii] / 100.0 / 180.0 * 3.141592653) * datr_dat2[ii] / 1000.0
            y = np.sin(datsita2[ii] / 100.0 / 180.0 * 3.141592653) * datr_dat2[ii] / 1000.0
        ...
        # ----get time MergDat_B_230305_192702.csv
        times = time.strftime('%y%m%d_%H%M%S', time.localtime(time.time()))
        with open(self.filepath + '/' + 'MergDatXY_' + str(times) + '.csv', 'w', encoding='utf-8', newline='') as \
                file_obj:
            writer = csv.writer(file_obj)
            title = ['X1','Y1','X2','Y2']
            writer.writerow(title)
            line = 0
            for line in range(1,len(xx)-100):
                contentWr = [ round(xx1[line],6), round(yy1[line],6), round(xx[line],6), round(yy[line],6)]
                writer.writerow(contentWr)
        
        # ----get time MergDat_B_230305_192702.csv
        with open(self.filepath + '/' + 'MergDatRaw_' + str(times) + '.csv', 'w', encoding='utf-8', newline='') as \
                file_obj:
            writer = csv.writer(file_obj)
            title = ['X1','Y1','X2','Y2']
            writer.writerow(title)
            line = 0
            for line in range(1,len(rawx2)-100):
                contentWr = [ round(rawx1[line],6), round(rawy1[line],6), round(rawx2[line],6), round(rawy2[line],6)]
                writer.writerow(contentWr)
    ...
    def readStdOfOneFile(self, filepath):
        with open(filepath) as f:
            ...
            datr_dat=[]
            ret=[]
            ret.append(float(rawcontent.split('X=')[1].split('\n')[0]) / 1000.0)
            ret.append(float(rawcontent.split('Y=')[1].split('\n')[0]) / 1000.0)
            ret.append(float(rawcontent.split('A=')[1].split('\n')[0]))
